...
int main()
{
   ...
   upc_forall (j=0; j<STREAM_ARRAY_SIZE; j++;&a[j])
    ...
    int remainder = STREAM_ARRAY_SIZE%(BLOCKSIZE);
    for (k=0; k<NTIMES; k++)
    {
        if(MYTHREAD == 0)
        {
        ...
        #else
            int bl = 0;
            for(bl=0; BLOCKSIZE*(MYTHREAD+THREADS*bl) < STREAM_ARRAY_SIZE; bl++)
            {
                int bbl = 0;
                double *locC   = (double*)(c        +BLOCKSIZE*(MYTHREAD+THREADS*bl));
			    double *locA   = (double*)(a        +BLOCKSIZE*(MYTHREAD+THREADS*bl));


                int bs=BLOCKSIZE;
                if(BLOCKSIZE*(MYTHREAD+THREADS*bl)+BLOCKSIZE>STREAM_ARRAY_SIZE && remainder!=0)
                {
                    //printf("\nreached remainder\n");
                    bs=remainder;
                }
                
                for(bbl=0;bbl<bs;bbl++)
                {
                    locC[bbl] = locA[bbl];
                }
            }

            /*upc_forall(j=0; j<STREAM_ARRAY_SIZE; j++;j)
            {
                c[j] = a[j];
            }*/
        #endif
        ...
        if(MYTHREAD == 0)
        {
            ...
            for(bl=0; BLOCKSIZE*(MYTHREAD+THREADS*bl) < STREAM_ARRAY_SIZE; bl++)
            {
                int bbl = 0;
                double *locC   = (double*)(c        +BLOCKSIZE*(MYTHREAD+THREADS*bl));
			    double *locB   = (double*)(b        +BLOCKSIZE*(MYTHREAD+THREADS*bl));
        ...
        {   
            ...
            for(bl=0; BLOCKSIZE*(MYTHREAD+THREADS*bl) < STREAM_ARRAY_SIZE; bl++)
            {
                ...
                for(bbl=0;bbl<bs;bbl++)
                {
                    locA[bbl] = locB[bbl]+scalar*locC[bbl];
                }
            }
        #endif
        upc_barrier;
...
void tuned_STREAM_Copy()
{
    ...
    upc_forall(j=0; j<STREAM_ARRAY_SIZE; j++)
    {
        c[j] = a[j];
    }
}
