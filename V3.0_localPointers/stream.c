...
void triadBetter(double* restrict locA, double* restrict locB, double* restrict locC);
...
shared int threadCounter[THREADS] = {0};

int main()
{
    a = (shared [BLOCKSIZE] double*)upc_all_alloc(THREADS, sizeof(STREAM_TYPE)*STREAM_ARRAY_SIZE/THREADS);
    b = (shared [BLOCKSIZE] double*)upc_all_alloc(THREADS, sizeof(STREAM_TYPE)*STREAM_ARRAY_SIZE/THREADS);
    c = (shared [BLOCKSIZE] double*)upc_all_alloc(THREADS, sizeof(STREAM_TYPE)*STREAM_ARRAY_SIZE/THREADS);
    ...
    STREAM_TYPE* locA = (STREAM_TYPE*)((shared [BLOCKSIZE] STREAM_TYPE *)a + MYTHREAD);
    STREAM_TYPE* locB = (STREAM_TYPE*)((shared [BLOCKSIZE] STREAM_TYPE *)b + MYTHREAD);
    STREAM_TYPE* locC = (STREAM_TYPE*)((shared [BLOCKSIZE] STREAM_TYPE *)c + MYTHREAD);
    ...
    for (k=0; k<NTIMES; k++)
    {
        ...
        int bl = 0;
        for(bl=0; bl<STREAM_ARRAY_SIZE/THREADS; bl++)
        {
            locC[bl] = locA[bl];
        }
