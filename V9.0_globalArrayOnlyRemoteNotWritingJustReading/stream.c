...
double *someLocalValue;
someLocalValue=malloc(sizeof(double)*(STREAM_ARRAY_SIZE));
for (k=0; k<NTIMES; k++)
{
    if(MYTHREAD == 0)
    {
    ...
    int bl = 0;
    // c[0] = a[0];
    for(bl=MYTHREAD; bl<STREAM_ARRAY_SIZE; bl = bl+THREADS)
    {
            int location = (bl+(THREADS/2))%STREAM_ARRAY_SIZE;
            someLocalValue[location] = a[location];
    }
    ...
    {
        int location = (bl+(THREADS/2))%STREAM_ARRAY_SIZE;
        someLocalValue[location] = scalar*c[location];
    }
    ...
    {
        int location = (bl+(THREADS/2))%STREAM_ARRAY_SIZE;
        someLocalValue[location] = a[location]+b[location];
    ...
    {
        int location = (bl+(THREADS/2))%STREAM_ARRAY_SIZE;
        someLocalValue[location] = b[location]+scalar*c[location];
    }
...
void checkSTREAMresults ()
{
	...
	if (sizeof(STREAM_TYPE) == 4) {
		...
		printf("WEIRD: sizeof(STREAM_TYPE) = %llu\n",sizeof(STREAM_TYPE));
		epsilon = 1.e-6;
	}
