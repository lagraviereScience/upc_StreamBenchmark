...
int main()
{
    ...
    int actualSize;
    actualSize = STREAM_ARRAY_SIZE / THREADS;

    a = (STREAM_TYPE*) malloc(sizeof(STREAM_TYPE)*(actualSize) + OFFSET);
    b = (STREAM_TYPE*) malloc(sizeof(STREAM_TYPE)*(actualSize) + OFFSET);
    c = (STREAM_TYPE*) malloc(sizeof(STREAM_TYPE)*(actualSize) + OFFSET);
...
{
	ssize_t j;
	upc_forall(j=0; j<STREAM_ARRAY_SIZE; j++;&c[j])
    {
	    b[j] = scalar*c[j];
    }
}
